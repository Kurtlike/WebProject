...
function drawFunc(dotset, colour){

    ctx.setLineDash([1, 0]);
    ctx.strokeStyle = colour;
    ctx.lineWidth = 3;

    ctx.moveTo(dotset[0].x, dotset[0].y);
    ctx.beginPath();
    for (let j = 0; j < dotset.length; j++) {
        let x = xNull + xScale * dotset[j].x;
        let y = yNull - yScale * dotset[j].y;
...
function reload(){
    ...
    createXAxis();
    createYAxis();
    drawFunc(funcDotsM1EX,"rgba(255,112,0,1)")
    drawFunc(funcDotsM1NM,"rgba(255,0,0,1)")
    drawFunc(funcDotsM2EX,"rgba(100,100,255,1)")
    drawFunc(funcDotsM2NM,"rgba(0,0,255,1)")
}
reload();
